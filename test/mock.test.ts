...
describe('per host mocking', () => 
  {
    it('express handler function', async () => 
      {
        let { text } = await supermock.post('https://google.com/auihwd?auwzgd=lawh').send();
    ...
    it('express router', async () => 
      {
        let { text } = await supermock.post('https://unolog.in/example1').send();

        assert.strictEqual(text, 'Route1');

        let { text: text2 } = await supermock.post('https://unolog.in/example2').send();
    ...
    it('express router', async () => 
      {
        let { text } = await supermock.post('https://example.com/example1').send();

        assert.strictEqual(text, 'Route1');

        let { text: text2 } = await supermock.post('https://example.com/example2').send();

        assert.strictEqual(text2, 'Route2');

        let { text: text3 } = await supermock.get('https://example.com/example3').send();
